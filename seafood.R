...
stock %>%
  ...
  mutate(
    value2 = ifelse(
      name == "share_of_fish_stocks_within_biologically_sustainable_levels_fao_2020",
      -1 * value,
      value
    )
  ) %>%
...
captured_vs_farmed %>%
  janitor::clean_names() %>%
  filter(entity %in% c("United States", "Spain", "Japan", "China", "India", "United Kingdom")) %>%
...
fishery %>%
  ...
  geom_line() + 
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Fishery Catches by Sector.",
       x = "Year",
       y = "Tons?",
...
production %>%
  ...
  pivot_longer(commodity_balances_livestock_and_fish_primary_equivalent_cephalopods_2766_production_5510_tonnes:commodity_balances_livestock_and_fish_primary_equivalent_marine_fish_other_2764_production_5510_tonnes) %>%
  mutate(name = case_when(name == "commodity_balances_livestock_and_fish_primary_equivalent_cephalopods_2766_production_5510_tonnes" ~ "cephalopods",
